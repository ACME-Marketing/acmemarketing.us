-- Initial database schema for ACME Marketing course system
...
CREATE POLICY "Episodes are insertable by authenticated users" ON episodes
    FOR INSERT WITH CHECK (auth.role() = 'authenticated');

CREATE POLICY "Episodes are updatable by authenticated users" ON episodes
    FOR UPDATE USING (auth.role() = 'authenticated');
...
-- Insert sample data
INSERT INTO courses (title, description, slug, price, is_free, enabled) VALUES
('AI Marketing Mastery', 'Learn how to leverage AI for effective marketing strategies', 'ai-marketing-mastery', 99.99, false, true),
('Marketing Automation Fundamentals', 'Master the basics of marketing automation', 'marketing-automation-fundamentals', 0.00, true, true),
('Content Strategy with AI', 'Create compelling content strategies using AI tools', 'content-strategy-with-ai', 149.99, false, true)
ON CONFLICT (slug) DO NOTHING;

-- Insert sample episodes
INSERT INTO episodes (course_id, title, description, episode_number, is_free, enabled) 
SELECT 
    c.id,
...
ON CONFLICT (course_id, episode_number) DO NOTHING;
...
CREATE INDEX IF NOT EXISTS idx_episodes_enabled ON episodes(enabled);
